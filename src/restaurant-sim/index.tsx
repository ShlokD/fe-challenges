...
const RestaurantSim = () => {
  ...
  const assignToFreeCook = (order: Order) => {
    ...
    setCooks(prev => {
      ...
      setTimeout(() => document.dispatchEvent(orderReadyEvent(order)), order.time)
      return newCooks;
    })
  }
    ...
    <div className="flex flex-col w-full">
      ...
      <main className="flex flex-col w-full min-h-screen p-4">
          ...
          <div className="flex flex-wrap w-2/3 gap-2 items-center justify-center">
            {customers.map((c, i) => {
              return (
                <p
                  key={`customer-${i}`}
                  className={`${c.state === "WAITING" ? "bg-black" : "bg-blue-400"
        ...
        >
          ...
          <h3 className="p-1 text-2xl font-bold">Available cooks: {cooks.length}</h3>
          <div className="flex  gap-2">
            <button aria-label="Add cook" className="p-4 text-2xl font-bold bg-black text-white rounded-lg" disabled={cooks.length === 8} onClick={addCook}>+</button>
          </div>
          {cooks.map((cook, i) => {
            if (cook === "0") {
              return <p className="text-xl p-2" key={`cook-${i}`}>Cook {i + 1} idle</p>
            }
            return <p className="text-xl p-2" key={`cook-${i}`}>Cook {i + 1} working  on order {cook}</p>
          })}
